                        ...
                        <div class="card-body text-center">
                            <div class="card-tittle">
                                ...
                                <div class="card-buttons">
                                        <a data-item=${cartItem.id} href="" class="card-button card-remove_btn" >Remove from Basket</a>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                document.querySelectorAll(".card-remove_btn").forEach(element => {
                    element.addEventListener("click", (e) => {
                        ...
                        let deleteFromBasket = async () => {
                            let data = await fetch(`http://localhost:8080/cart/delete/${selectedProductId}?token=${localStorage.getItem("token")}`, {
                                method: "DELETE",
                            })
                        }
                        deleteFromBasket()
...
orderBtn.addEventListener("click", async () => {
    let data = await fetch("http://localhost:8080/order/create-checkout-sessions", {
        method: "POST",
        mode: "cors",
        headers: {
